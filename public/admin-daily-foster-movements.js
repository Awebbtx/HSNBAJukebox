...
function groupByDirection(rows) {
  ...
  for (const row of rows || []) {
    const key = `${row.outOrIn || "Unknown"}`.trim() || "Unknown";
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(row);
  }
  return Array.from(groups.entries()).map(([outOrIn, items]) => ({
    outOrIn,
    items
  }));
}
...
function renderGroup(group) {
  ...
  return RC.group(group.outOrIn, group.items.length, cards.join(""));
}

function renderTable(rows) {
  if (!rows.length) {
    els.tableWrap.innerHTML = '<span style="color:var(--muted)">No daily foster movements found.</span>';
    return;
  }

  const groups = groupByDirection(rows)
    .sort((a, b) => a.outOrIn.localeCompare(b.outOrIn));

  els.tableWrap.innerHTML = groups.map((group) => renderGroup(group)).join("\n");
}

async function loadReport() {
  els.metaText.textContent = "Loading report...";
  try {
    const data = await api("/api/admin/reporting/daily-foster-movements");
    ...
    els.metaText.textContent = error.message || "Failed to load Daily Foster Movements report.";
